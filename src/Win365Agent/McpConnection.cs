...
public sealed class McpConnection(HttpClient http, IAgentUserTokens tokens, Settings settings)
{
    private int sequence;
    private string version = "2025-06-18";
    private string? transportSession;
    public async Task InitializeAsync(CancellationToken ct)
    {
        var result = await SendAsync("initialize", new
        {
            protocolVersion = version, capabilities = new { },
            clientInfo = new { name = "foundry-w365-sample", version = "1.0.0" }
        }, false, ct);
        version = result.GetProperty("protocolVersion").GetString()!;
        if (version is not ("2025-06-18" or "2025-03-26"))
    ...
    private async Task<JsonElement> SendAsync(string method, object parameters, bool notification, CancellationToken ct)
    {
        ...
        request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", (await tokens.GetAsync(AgentUserTokens.Atg, ct)).Token);
        request.Headers.Accept.ParseAdd("application/json, text/event-stream");
        request.Headers.Add("MCP-Protocol-Version", version);
        if (transportSession is not null) request.Headers.Add("Mcp-Session-Id", transportSession);
        ...
        if (response.Headers.TryGetValues("Mcp-Session-Id", out var sessions))
            transportSession = sessions.Single();
        if (notification) return default; // Initialized is allowed to return an empty 202/204.
        ...
        if (response.Content.Headers.ContentType?.MediaType == "text/event-stream")
        {
            ...
            {
                total += line.Length;
                if (total > 4 * 1024 * 1024) throw new InvalidOperationException("MCP response exceeded 4 MiB.");
                if (line.Length == 0)
                {
                    if (data.Length > 0 && Parse(data.ToString(), id) is { } result) return result;
                    data.Clear();
                }
                else if (line.StartsWith("data:", StringComparison.Ordinal))
                    data.AppendLine(line[5..].TrimStart(' '));
            }
            if (data.Length > 0 && Parse(data.ToString(), id) is { } final) return final;
        ...
        {
            if (buffer.Length + count > 4 * 1024 * 1024) throw new InvalidOperationException("MCP response exceeded 4 MiB.");
    ...
    internal static JsonElement? Parse(string text, int id)
    {
        ...
        if (!root.TryGetProperty("id", out var value) || !value.TryGetInt32(out var actual) || actual != id) return null;
        if (root.TryGetProperty("error", out _)) throw new InvalidOperationException("MCP returned a JSON-RPC error; operation is not replayed.");
        ...
        if (node.ValueKind == JsonValueKind.Object)
        {
            ...
            foreach (var property in node.EnumerateObject())
            {
                if (property.Name == "text" && property.Value.ValueKind == JsonValueKind.String)
                {
                    ...
                    {
                        try { using var embedded = JsonDocument.Parse(text); if (Field(embedded.RootElement, name) is { } found) return found; }
                        catch (JsonException) { /* Ordinary tool text is not necessarily JSON. */ }
                    }
                }
                if (Field(property.Value, name) is { } nested) return nested;
            }
        }
        else if (node.ValueKind == JsonValueKind.Array)
            foreach (var item in node.EnumerateArray()) if (Field(item, name) is { } found) return found;
        return null;
    }
}
