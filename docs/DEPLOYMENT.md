...
| Teardown reports a missing layer deployment or an already-deleted resource group | Rerun `azd down`; cleanup treats viewer RBAC beneath a confirmed-missing resource group as already absent. Use `Invoke-AzdDown.ps1` if a layer deployment is missing. |
